...
function parseModelPayload(body) {
    if (!isPlainObject(body) || !("model" in body))
        return { ok: false, message: 'Expected JSON body with string or null field "model".' };
    ...
    if (model && !isProviderQualifiedModelId(model))
        return { ok: false, message: 'Invalid role-generation model. Expected a provider-qualified model id such as "provider/model".' };
