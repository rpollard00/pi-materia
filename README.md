...
Both loadouts are defined entirely as config using top-level reusable materia prompts plus socket adapters for JSON parsing, state assignment, conditional edges, foreach cursors, and named Materia assignments. Bundled default socket ids are sequential (`Socket-1` through `Socket-6`); materia identity stays in socket fields such as `materia` or `utility`, while displays can add context like `Socket-4 (Build)`. Loadout config and WebUI save payloads must use `sockets`; legacy loadout or loop `nodes` fields are rejected with guidance to use `sockets` instead. Saved cast artifacts/events may still use historical node names for compatibility. Use `/materia loadout` to see which one is active and `/materia loadout Full-Auto` or `/materia loadout Planning-Consult` to switch.
