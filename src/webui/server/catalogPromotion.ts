import {
  ...
  type CatalogLocalActionResult,
} from '../../application/catalogActions.js';
import {
...
export type MateriaPromoteCatalogCallback = (
  request: CatalogLocalActionRequest,
) => Promise<CatalogLocalActionResult>;

export interface CatalogPromotionRouteDeps {
  promoteCatalog?: MateriaPromoteCatalogCallback;
  /** Reload the merged runtime config after a successful local write. */
 ...
 */
export async function handleCatalogPromotionRoute(
  req: IncomingMessage,
  res: ServerResponse,
  deps: CatalogPromotionRouteDeps,
): Promise<void> {
  ...
  let request: CatalogLocalActionRequest;
  try {
...
function readRouteAction(rawUrl: string | undefined): CatalogLocalAction | undefined {
  const pathname = new URL(rawUrl ?? '', 'http://localhost').pathname;
  if (!pathname.startsWith(`${CATALOG_PROMOTION_PATH_PREFIX}/`)) return undefined;
  const suffix = pathname.slice(CATALOG_PROMOTION_PATH_PREFIX.length + 1);
  if (suffix.length === 0 || suffix.includes('/') || !isCatalogLocalAction(suffix)) return undefined;
  return suffix;
}

function parsePromotionRequest(body: unknown, action: CatalogLocalAction): CatalogLocalActionRequest {
  if (!isPlainObject(body)) throw new TypeError('Catalog promotion request body must be a JSON object.');
...
function conflictDetails(
  request: CatalogLocalActionRequest,
  result: Extract<CatalogLocalActionResult, { status: 'needs_confirmation' | 'rejected' }>,
  code: string,
...
function provenanceDetails(
  origin: CatalogOriginProvenance | undefined,
  previousOrigin: CatalogOriginProvenance | undefined,
): Record<string, CatalogOriginProvenance> {
  return {
...
function readPromotedDefinitionDrift(
  config: unknown,
  kind: CatalogLocalActionResult['kind'],
  localKey: string,
): unknown {
  if (!isPlainObject(config) || !isPlainObject(config.catalogDrift)) return undefined;
...
function sendPromotionError(res: ServerResponse, status: number, code: string, message: string): void {
