...
export type MateriaSetDefaultLoadoutFailureCode = 'invalid_name' | 'unknown_loadout' | 'unavailable';

export type MateriaSetDefaultLoadoutResult =
  | {
    ok: true;
  ...
  | {
    ok: false;
    code: MateriaSetDefaultLoadoutFailureCode;
    message: string;
    defaultLoadoutId?: string | null;
  };

export type MateriaSetDefaultLoadoutCallback = (name: string | null) => Promise<MateriaSetDefaultLoadoutResult>;

export interface DefaultLoadoutRouteDeps {
  setDefaultLoadout?: MateriaSetDefaultLoadoutCallback;
}

function failureStatus(code: MateriaSetDefaultLoadoutFailureCode): number {
  if (code === 'invalid_name') return 400;
  if (code === 'unknown_loadout') return 404;
  return 503;
}

function sendDefaultLoadoutError(res: ServerResponse, status: number, code: string, message: string, extras: Record<string, unknown> = {}) {
  sendJson(res, status, { ok: false, error: { code, message }, ...extras });
}

export async function handleDefaultLoadoutRoute(req: IncomingMessage, res: ServerResponse, deps: DefaultLoadoutRouteDeps) {
  if (req.method !== 'POST') {
