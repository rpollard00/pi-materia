...
function GraphToolbar({ createLoopDisabled, createTaskIteratorLoop, loadoutNameInput, selectedLoopSocketIds, setLoadoutNameInput, commitActiveLoadoutRename, socketLabel }: GraphToolbarProps) {
  return (
...
function EdgeLayer({ activeLoadout, height, loopRegions, materia, routedEdges, toggleEdgeCondition, toggleLoopExitCondition, width }: EdgeLayerProps) {
  return (
    <svg className="loadout-edge-layer" width={width} height={height} aria-label="Loadout edges">
      ...
      {routedEdges.map(({ edge, path, labelX, labelY, labelRotate, routeClass }) => {
        ...
        return (
          <g key={edge.id} data-testid={edgeTestId} data-edge-kind={edge.kind} role="button" tabIndex={0} aria-label={`${edgeLabel} from ${edge.from} to ${edge.to}`} className={`loadout-edge loadout-edge-${conditionClass} loadout-edge-route-${routeClass} ${isGeneratorInput ? 'loadout-edge-generator-input' : ''} ${isLoopExitEdge ? 'loadout-edge-loop-exit' : ''} loadout-edge-clickable`} onClick={activateEdge} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); activateEdge(); } }}>
...
function LoopEditorPanel({ activeLoadout, breakLoop, clearLoopExit, socketDisplayLabel, socketLabel, updateLoopExit }: LoopEditorPanelProps) {
...
function SocketActionModal(props: SocketActionModalProps) {
  const {
    ...
    socketActionMode, socketPropertyError, socketPropertyForm, closeSocketActionModal, createConnectedSocket,
  ...
  if (!socketActionId || !activeLoadout?.nodes?.[socketActionId]) return null;
