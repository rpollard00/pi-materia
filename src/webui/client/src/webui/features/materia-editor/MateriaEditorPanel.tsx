...
export function MateriaEditorPanel({ controller, toolRegistry }: MateriaEditorPanelProps) {
  const { form, modelOptions, colorPicker, roleGeneration, persistence, selector } = controller;
  const { materiaForm } = form;

  return (
    <section className="materia-editor-workspace" aria-label="Materia creation editor">
      <MateriaSelectorSidebar
        items={selector.items}
        selectedId={materiaForm.editingSocketId || undefined}
        onSelect={form.editMateria}
        onNew={() => {
          form.resetMateriaEditorForm();
          roleGeneration.discardGeneratedRolePrompt();
        }}
        onDuplicate={selector.duplicateMateria}
        onToggleLock={selector.setMateriaLockState}
        onDelete={selector.deleteMateria}
      />

      <div className="fantasy-panel materia-editor-main p-4 sm:p-6">
        <div className="mb-5">
        ...
        <MateriaEditorActions form={form} roleGeneration={roleGeneration} persistence={persistence} selectedPolicy={selector.selectedPolicy} />
      </div>
    </section>
  );
}
