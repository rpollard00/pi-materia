...
export function hasBuiltInUtility(alias) {
    return typeof alias === "string" && Object.prototype.hasOwnProperty.call(registry, alias);
}
export async function executeBuiltInUtility(alias, input) {
    const utility = registry[alias];
    if (!utility)
        throw new Error(`Unknown utility alias "${alias}".`);
    return await utility(input);
}
async function ensureIgnored(input) {
    const params = input.params;
    const rawPatterns = params.patterns;
    const patterns = Array.isArray(rawPatterns) ? rawPatterns.filter((value) => typeof value === "string" && value.trim().length > 0).map((value) => value.trim()) : [];
    if (patterns.length === 0)
        throw new Error('project.ensureIgnored requires params.patterns to be a non-empty string array.');
...
async function detectVcs(input) {
