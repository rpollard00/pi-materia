...
export async function loadConfig(cwd, configuredPath) {
...
async function loadConfigFile(file, source = file) {
    if (!existsSync(file))
        throw new Error(`pi-materia config file not found: ${file}`);
...
async function mergeConfig(parsed) {
...
async function loadDefaultConfig() {
    return JSON.parse(await readFile(getBundledDefaultConfigPath(), "utf8"));
}
export function resolveArtifactRoot(cwd, artifactDir) {
    return artifactDir ? resolveFromCwd(cwd, artifactDir) : path.join(cwd, ".pi", "pi-materia");
}
export function resolveFromCwd(cwd, inputPath) {
