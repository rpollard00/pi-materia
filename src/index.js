...
export default function piMateria(pi) {
    ...
    pi.on("context", (event, ctx) => {
        ...
        if (!state?.active || !state.awaitingResponse)
            return;
        return {
            messages: buildIsolatedMateriaContext(event.messages, state),
        };
    });
        ...
        handler: async (args, ctx) => {
            ...
            if (subcommand === "continue") {
                try {
                    const state = loadActiveCastState(ctx);
                    if (!state)
                        throw new Error("No pi-materia cast state in this session.");
...
async function renderCastList(artifactRoot) {
    let names;
    try {
    ...
    const valid = casts.filter((cast) => Boolean(cast)).sort((a, b) => b.modified - a.modified);
    return [
        "Materia Casts",
...
async function readCastSummary(id, dir) {
    ...
    const ok = end?.data && typeof end.data === "object" ? end.data.ok : undefined;
    return {
        id,
        dir,
        modified,
        status: ok === true ? "complete" : ok === false ? "failed" : "active/unknown",
        request: manifest?.request ?? start?.data?.request,
    };
}
...
function getConfiguredConfigPath(pi) {
