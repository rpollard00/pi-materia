...
export { parseQuestListArgs, parseQuestMoveArgs };
    ...
    handler: async (args, ctx) => {
      ...
      if (subcommand === "revive") {
        try {
          ...
          if (!castId) {
            ctx.ui.notify("No failed or aborted pi-materia casts are available to revive. Use /materia recast [cast-id] for casts that were recast previously.", "info");
            return;
          }
        } catch (error) {
