...
- Proactive compaction is checked only in `src/native.ts:sendMateriaTurn` before writing the context artifact and before sending the hidden prompt. It calls `maybeRunProactiveCompaction`, which uses only `ctx.getContextUsage().percent` and a fixed threshold, then calls `ctx.compact(...)` when the reported percentage is high enough.
