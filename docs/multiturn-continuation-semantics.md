...
1. `startNativeCast()` creates an active `MateriaCastState`, sets `awaitingResponse = true` and socket state (`nodeState` legacy DTO field) = `"awaiting_agent_response"`, then calls `startSocket()` for the pipeline entry.
2. `startSocket()` increments the socket visit, records `node_start`, applies materia model/tool scope for agent sockets, and sends the hidden materia prompt with `sendMateriaTurn(..., { triggerTurn: true })`.
3. `handleAgentEnd()` processes the newest assistant entry. For normal single-turn sockets it immediately calls `completeSocket()`.
4. For `agent` sockets whose resolved materia has `multiTurn: true`, `handleAgentEnd()` records the assistant output as a `node_refinement` artifact, sets socket state (`nodeState` legacy DTO field) = `"awaiting_user_refinement"`, clears `awaitingResponse`, saves state, updates status/widgets, and notifies the user to refine the draft or run `/materia continue` to finalize. The socket pauses after each assistant turn until that command is run.
5. While paused, ordinary user messages are treated as refinement instructions. The normal Pi turn continues; the `before_agent_start` hook calls `prepareMultiTurnRefinementTurn()` to restore the active materia/model/tools, set `awaitingResponse = true` and socket state (`nodeState` legacy DTO field) = `"awaiting_agent_response"`, record `context_refinement`, and let the user's message drive another isolated refinement turn.
...
- Text sockets write the normal `node_output` artifact via `recordNodeOutput()`.
