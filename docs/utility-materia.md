...
`Blackbelt-Bootstrap` is the deterministic replacement for planner-created VCS setup tasks in bootstrap flows. It requires `jj` on `PATH`; if `jj` is missing, the utility hard-fails and writes a `state.blackbeltBootstrap` failure payload. When run in a directory that is not already a jj repository, it runs `jj git init` from the current project directory (after ensuring common build-artifact directories are gitignored), including when a plain Git repository already exists, and does not use colocated mode.

After detecting or initializing the jj repository, bootstrap probes whether the current working commit (`@`) is empty using `jj log -r @ -T empty` — jj's canonical emptiness signal, chosen over `jj diff --summary` because the bounded `true`/`false` output cannot blow past execFile's `maxBuffer` on repos with many untracked build artifacts. The bootstrap is **idempotent**:

- When `@` is already empty, bootstrap does not describe, does not run `jj new`, and only places the `blackbelt/` bookmark on `@`. An already-empty repository therefore receives no unnecessary commit.
- When `@` is non-empty, pre-existing work is **preserved**: bootstrap describes the dirty revision with a deterministic `bootstrap: <bookmarkName>` message, sets the `blackbelt/` bookmark on that described revision (so the bookmark always tracks a described, pushable commit), then runs `jj new` to open a fresh empty working commit and moves `@` onto it.

Before reporting success, bootstrap re-probes `@` and **verifies** it is empty. An unverified probe (for example, `jj log` inspection failing) or a confirmed non-empty commit fails the cast rather than reporting empty success, because downstream materias rely on `emptyHead` meaning a verified empty working commit. The guaranteed postcondition is that a successful cast containing `Blackbelt-Bootstrap` continues from a verified-empty `@`, with any pre-existing work anchored under the `blackbelt/` bookmark.

The utility writes deterministic state under `state.blackbeltBootstrap` (including `ok`, `root`, `available`, `initialized`, `newWorkingCommit`, `emptyHead`, and `bookmarkName` on success). Field meanings:

- `initialized` — `true` only when bootstrap ran `jj git init` itself; a pre-existing jj repo leaves it `false`.
- `newWorkingCommit` — `true` only when bootstrap actually ran `jj new` to open a fresh empty commit; a clean `@` leaves it `false`.
- `emptyHead` — `true` only when the post-bootstrap `@` was positively verified empty by `jj log -r @ -T empty`, never on an unverified probe.

The `bookmarkName` is a deterministic git-ref-safe name under the `blackbelt/` prefix shaped like `blackbelt/<noun>-<verb>-<short-hash>` (for example, `blackbelt/crystal-casts-1a2b3c4d5e`) that `Blackbelt-Maintain` reads to advance the bookmark with each checkpoint. Naming priority: explicit sanitized `params.bookmarkName` override, otherwise a SHA-256-derived noun/verb/hash name seeded by `input.castId`; `runDir` and `cwd` are used only as last-resort seeds when `castId` is absent. Generated names do not expose timestamps or raw cast ids. This utility state patch is separate from agent-authored handoff JSON; do not ask planners, builders, or maintainers to emit those fields as agent JSON.
